...
calculator_version = "Calculator v1.4"
calculator_version_only = calculator_version.replace("Calculator v", "")
calculator_version_only = float(calculator_version_only)
...
calculator_icon = resource_path("Calculator.ico")
about_icon = resource_path("Info.ico")
s_Calculator = resource_path("Scientific Calculator.ico")
splash_screen_Calculator_png =  resource_path("Splash_Screen.png")
whats_new_icon = resource_path("What's new.ico")
bmi_icon = resource_path("BMI.ico")

v1_4_what_new_list = "- \"Update\" tab added\n- \"Check for Update\" section added under \'Update\' tab\n" \
                     "- In Application update system"
...
def check_for_update():
    ...
    updator.iconbitmap(about_icon)
    updator.geometry("350x120")
    updator.resizable(0, 0)

    lbl1 = Label(updator, text="Please check your \nInternet connection😑", font=('Arial', 15), justify=LEFT)
    lbl1.place(y=50)
    lbl1.pack(anchor=CENTER, pady=30)

    URL = "https://github.com/Sayad-Uddin-Tahsin/Simple-Calculator/blob/main/Current%20Version"
    ...
    final = dom.xpath('//*[@id="LC1"]')[0].text
    final_ = float(final)

    def download_now():
        url = "https://github.com/Sayad-Uddin-Tahsin/Simple-Calculator/raw/main/Calculator%20all%20exe/Calculator%20v1.2%20Setup.exe"
        url_ = url.replace("1.2", final)
        webbrowser.open(url_)

    if final_ > calculator_version_only:
        lbl1.config(text="Update Available!", font=("Arail", 20))
        btn = Button(updator, text="Download now", font=('Arial', 10), command=download_now)
        # btn.pack(side=RIGHT, anchor="se")
        btn.place(x=127, y=75)

    else:
        lbl1.config(text="You are up to date! 😊", font=('Arial', 15))
...
def whats_new():
    ...
    v1_4 = Label(whats_new_window, text="Version 1.4", justify=LEFT, font=("Arial", 12), relief=FLAT, state=ACTIVE)
    v1_4_what_new_label = Label(whats_new_window, text=v1_4_what_new_list, justify=LEFT, font=("Arial", 9),
                                relief=FLAT, state=ACTIVE)

    v1_3 = Label(whats_new_window, text="Version 1.3", justify=LEFT, font=("Arial", 12), relief=FLAT, state=DISABLED)
    v1_3_what_new_label = Label(whats_new_window, text=v1_3_what_new_list, justify=LEFT, font=("Arial", 9),
                                relief=FLAT, state=DISABLED)

    lbl1.place(x=10)
    v1_4.place(x=10, y=45)
    v1_4_what_new_label.place(x=10, y=70)

    v1_3.place(x=10, y=145)
    v1_3_what_new_label.place(x=10, y=170)
